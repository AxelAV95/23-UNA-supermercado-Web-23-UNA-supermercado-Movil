...
CREATE DEFINER=`root`@`localhost` PROCEDURE `modificarCategoria` (IN `id` INT, IN `nombre` VARCHAR(255))   BEGIN
    ...
    IF (SELECT COUNT(*) FROM tbcategoria WHERE categorianombre = nombre AND categoriaid <> id) > 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El nombre ya está en uso.';
    ELSE
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `modificarProducto` (IN `p_id` INT, IN `p_nombre` VARCHAR(255), IN `p_precio` DECIMAL(10,2), IN `p_fecha_ingreso` DATE, IN `p_stock` INT, IN `p_estado` VARCHAR(50), IN `p_categoria_id` INT, IN `p_proveedor_id` INT)   BEGIN
    ...
    -- Verificar si el nombre ya está en uso
    IF (SELECT COUNT(*) FROM tbproducto WHERE productonombre = p_nombre AND productoid <> p_id) > 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El nombre ya está en uso.';
    ELSE
        -- Actualizar el producto
        UPDATE tbproducto
        SET
            productonombre = p_nombre,
...
(1, 1010101, 'Juan', 'Péres', 88888888, 'San José', '2022-05-13', '0000-00-00', 1, 2),
...
(1, 'Distribuidora Salazar ', 'Calle 4, Avenida 12, Heredia', 27778888, 'salazar@gmail.com', '10.0000', '-84.1167'),
(2, 'Los Patitos', 'Rio frio', 89898989, 'pa@com.cr', '89087787', '7478744'),
(3, 'Alimentos C', 'Calle 3, Alajuela', 88889999, 'info@alimentosC.com', '10.011089', '-84.212752'),
(4, 'Los Reyes', 'Guapiles', 98890987, 'Rey@una.cr', '7678877', '34545454'),
(5, 'Orosi', 'Ticaban', 67899876, 'oro@a.com', '454545', '545454');
...
--
ALTER TABLE `tbdescuento`
  MODIFY `descuentoid` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;

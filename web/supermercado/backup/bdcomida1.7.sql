...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 09-11-2022 a las 09:45:13
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `actualizarEstadoOrden` (IN `id` INT, IN `estado` INT)  NO SQL BEGIN
	DECLARE error INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminarCategoria` (IN `id` INT)  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminarOrden` (IN `id` INT)  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminarProducto` (IN `id` INT)  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminarTipoUsuario` (IN `id` INT)  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eliminarUsuario` (IN `id` INT)  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarCategoria` (IN `id` INT, IN `descripcion` VARCHAR(255), IN `img` VARCHAR(500), IN `codigo` INT)  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarDetalle` (IN `id` INT, IN `ordenid` INT, IN `productoid` INT, IN `precio` INT, IN `cantidad` INT)  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarOrden` (IN `id` INT, IN `cliente` VARCHAR(255), IN `telefono` INT, IN `correo` VARCHAR(255), IN `metodo` INT, IN `fecha` DATE, IN `total` INT, IN `estado` INT)  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarProducto` (IN `id` INT, IN `img` VARCHAR(255), IN `nombre` VARCHAR(255), IN `precio` INT, IN `estado` INT, IN `categoria` INT, IN `codigo` INT)  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarTipoUsuario` (IN `id` INT, IN `descripcion` VARCHAR(255))  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarUsuario` (IN `id` INT, IN `nombre` VARCHAR(255), IN `telefono` INT, IN `correo` VARCHAR(255), IN `password` VARCHAR(255), IN `tipo` INT)  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertarUsuarioOrden` (IN `idorden` INT, IN `idusuario` INT)  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `modificarCategoria` (IN `id` INT, IN `descripcion` VARCHAR(255), IN `img` VARCHAR(500))  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `modificarProducto` (IN `id` INT(20), IN `img` VARCHAR(255), IN `nombre` VARCHAR(255), IN `precio` INT(20), IN `estado` INT(20), IN `categoria` INT(20), IN `codigo` INT(20))  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `modificarTipoUsuario` (IN `descripcion` VARCHAR(255), IN `id` INT)  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `modificarUsuario` (IN `id` INT(20), IN `nombre` VARCHAR(255), IN `telefono` INT(20), IN `correo` VARCHAR(255), IN `password` VARCHAR(255), IN `tipo` INT(20))  NO SQL BEGIN
	DECLARE error INT;
	DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
    	GET CURRENT DIAGNOSTICS CONDITION 1 error = MYSQL_ERRNO;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerCategorias` ()  NO SQL SELECT * FROM `tbcategoria`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerDetallesOrden` (IN `ordenid` INT)  NO SQL SELECT `detalleid`,`detalleordenid`, `productonombre`,`detalleprecio`,`detallecantidad` FROM tbdetalle INNER JOIN tborden ON tbdetalle.detalleordenid = tborden.ordenid INNER JOIN tbproducto ON tbproducto.productoid = tbdetalle.detalleproductoid WHERE tborden.ordenid = ordenid$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerOrdenes` ()  NO SQL SELECT * FROM `tborden` ORDER BY `ordenestado` = 2 DESC$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerPaginasProducto` (IN `inicio` INT, IN `limite` INT)  NO SQL SELECT * FROM tbproducto LIMIT inicio, limite$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `ObtenerPaginasProductoCategoria` (IN `inicio` INT, IN `limite` INT, IN `categoria` INT)  NO SQL SELECT * FROM tbproducto WHERE productocategoriaid = categoria LIMIT inicio, limite$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerProductos` ()  NO SQL SELECT * FROM `tbusuario`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerTipoUsuario` ()  NO SQL SELECT * FROM `tbtipousuario`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerTotalGanancias` ()  NO SQL SELECT SUM(`ordentotal`) as totalganancias FROM `tborden`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerTotalOrdenes` ()  NO SQL SELECT COUNT(*) totalordenes FROM `tborden`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerTotalProductos` ()  NO SQL SELECT COUNT(*) as totalproductos FROM `tbproducto`$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `obtenerUsuarios` ()  NO SQL SELECT * FROM tbusuario$$
...
(5, 'img/productos/5/892.png', 'Cafe', 500, 1, 1, 5);
...
(1, 'Administrador', 78989092, 'admi@hotmail.com', '2345ssd', 1),
(2, 'Axel', 33333, 'villalobos.axel@yahoo.es', '12345', 1),
(3, 'Edwin', 43434343, 'ed@ji.com', 'erer44', 2);
